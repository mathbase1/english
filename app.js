...
(function (root, factory) {
  ...
  const STORAGE_KEY = "gcse_english_github_pages_answer_v10";
  ...
  function createMarker(config, rules, options) {
    ...
    function detectAnchorProfile(textNorm) {
      const labels = ["generic", "clear", "developed", "perceptive", "unsupported"];
      const scores = {};
    ...
    function classifyUnit(unitObj, previousUnit, unitIndex) {
      ...
      const analysisLanguage = strongVerbHits.length > 0 ||
        ...
        /\bseem\b|\bseems\b|\bfeel\b|\bfeels\b|\bshowing\b|\bsuggesting\b|\bhighlighting\b|\breflects\b|\bmirrors\b|\bconveys\b|\breveals\b/.test(norm);
      const meaningfulCommentary = commentaryDensity >= 0.16 || analysisLanguage || methodMentions.length > 0 || hedging.softHits.length > 0 || hasSpecificEffect || hasSupportedInterpretation;
      ...
      const quoteHeavyNoComment = hasDirectEvidence && commentaryDensity < 0.13 && !analysisLanguage && !hasWriterChoice && !methodMentions.length;
      ...
      let qualityScore = 0;
      if (hasEvidenceCarrier) qualityScore += 0.85;
      if (hasSpecificEffect) qualityScore += 0.75;
      if (hasSupportedInterpretation) qualityScore += 0.9;
      if (hasHighEffect) qualityScore += 0.45;
      if (hasInterpretiveSignal) qualityScore += 0.35;
      if (hasWriterChoice) qualityScore += 0.4;
      if (strongVerbHits.length) qualityScore += 0.35;
      if (threadHits.length) qualityScore += 0.25 * Math.min(threadHits.length, 2);
      if (anchorProfile.clear > 0.55) qualityScore += 0.15;
      if (anchorProfile.developed > 0.55) qualityScore += 0.24;
      if (anchorProfile.perceptive > 0.6) qualityScore += 0.3;
      if (methodEval.correct.length) qualityScore += 0.18;
      if (commentaryDensity >= 0.22) qualityScore += 0.2;

      qualityScore -= severeGenericCount * 0.58;
      qualityScore -= mildGenericCount * 0.06;
      qualityScore -= genericSignals.featureHits.length * 0.24;
      qualityScore -= hardUncertaintyCount * 0.52;
      qualityScore -= Math.max(0, softHedgeCount - 1) * 0.08;
      qualityScore -= contradictions.length * 0.8;
      qualityScore -= unsupported.length * 1.05;
      qualityScore -= methodEval.serious.length * 0.48;
      qualityScore -= featureSpotting ? 0.6 : 0;
      qualityScore -= quoteDump ? 0.75 : 0;
      qualityScore -= copiedUnit ? 1.1 : 0;
      qualityScore -= quoteHeavyNoComment ? 0.55 : 0;
      qualityScore -= overlyLongEvidence && !hasSupportedInterpretation ? 0.24 : 0;
      qualityScore -= tooManyQuotes && commentaryDensity < 0.42 ? 0.38 : 0;
      qualityScore -= tooManyQuotes && !(hasWriterChoice || grounded.comparisonHits.length > 0 || threadHits.length > 0) ? 0.22 : 0;
      qualityScore -= genericSignals.genericOpening && !(hasSupportedInterpretation || hasHighEffect || hasWriterChoice) ? 0.06 : 0;
      ...
      if (mostlyNeutral) {
        ...
        (anchorProfile.perceptive >= Math.max(anchorProfile.clear, anchorProfile.developed) * 0.95 || threadHits.length >= 2 || grounded.comparisonHits.length > 0) &&
        !tooManyQuotes &&
        severeGenericCount === 0 &&
        ...
        (hasWriterChoice || grounded.comparisonHits.length > 0 || strongVerbHits.length > 0 || threadHits.length >= 1 || anchorProfile.developed > 0.6) &&
        ...
        meaningfulCommentary &&
        qualityScore >= 1.62 &&
        (hasSpecificEffect || hasSupportedInterpretation || hasWriterChoice || strongVerbHits.length > 0 || weakVerbHits.length > 0 || analysisFragments >= 1 || anchorProfile.clear > 0.54)
      ) {
        ...
        meaningfulCommentary &&
        qualityScore >= 0.95 &&
        (hasSpecificEffect || weakVerbHits.length > 0 || hasSupportedInterpretation)
      ) {
      ...
      if (classification === "developed_analysis" && !hasHighEffect && !hasWriterChoice && grounded.comparisonHits.length === 0 && threadHits.length === 0 && commentaryDensity < 0.45) {
        classification = "clear_analysis";
      }
      ...
      if (classification === "clear_analysis" && severeGenericCount > 0 && !hasHighEffect && !hasSupportedInterpretation && anchorProfile.clear < 0.56) {
        classification = "simple_analysis";
      }
    ...
    function evaluateThreads(units) {
      ...
      return {
        ...
        dominantThreadCount: Object.values(analyticalCounts).length ? Math.max.apply(null, Object.values(analyticalCounts)) : 0
      };
    }
    ...
    function evaluateFocus(aggregate) {
      ...
      const positive = (
        aggregate.perceptiveCount * 1.4 +
        aggregate.developedCount * 1.15 +
        aggregate.clearCount * 0.95 +
        aggregate.simpleCount * 0.62 +
        aggregate.literalCount * 0.22
      );
      const drag = (
        aggregate.genericCount * 0.42 +
        aggregate.featureSpottingCount * 0.5 +
        aggregate.referenceOnlyCount * 0.32 +
        aggregate.quoteDumpCount * 0.7 +
        aggregate.unsupportedCount * 0.9 +
    ...
    function computeDimensionScores(aggregate) {
      ...
      const evidenceQuality = clamp(
        aggregate.evidence.attachedCount * 0.7 +
        aggregate.evidence.usableCount * 0.9 +
        aggregate.evidence.selectiveCount * 1.02 +
        aggregate.evidence.judiciousCount * 0.75 -
        aggregate.evidence.dumpedCount * 0.72 -
        Math.max(0, aggregate.referenceOnlyCount - 1) * 0.14,
        0,
        4.4
      );
      const coverageQuality = clamp(
        aggregate.coverage.supportedGroups.length * 0.62 +
        aggregate.coverage.analyticalGroups.length * 0.84 +
        aggregate.coverage.interpretiveGroups.length * 0.96 -
        Math.max(0, aggregate.coverage.referenceGroups.length - aggregate.coverage.supportedGroups.length) * 0.06,
        0,
        4.4
      );
      const explanationQuality = clamp(
        aggregate.clearEquivalent * 0.9 +
        aggregate.analysisFragmentCount * 0.14 +
        aggregate.simpleCount * 0.16 -
        aggregate.featureSpottingCount * 0.34 -
        aggregate.genericCount * 0.1 -
        aggregate.quoteDumpCount * 0.35,
        0,
        4.4
      );
      const interpretationDepth = clamp(
        ...
        aggregate.coverage.interpretiveGroups.length * 0.36 +
        aggregate.threadData.sustainedCount * 0.42 +
        aggregate.correctMethodLinks * 0.06 -
        aggregate.unsupportedCount * 0.9 -
        aggregate.literalCount * 0.14 -
        aggregate.hardUncertaintyCount * 0.14,
        0,
        4.4
      );
      ...
      const control = clamp(
        aggregate.focusScore * 0.9 + (aggregate.creditworthyCount / Math.max(aggregate.totalUnits || 1, 1)) * 0.95 - weakRatio * 0.45 - aggregate.genericOpeningCount * 0.05,
        0,
        4
      );
      const taskFocus = clamp(
        aggregate.focusScore + (aggregate.coverage.supportedGroups.length >= 2 ? 0.2 : 0) - aggregate.genericOpeningCount * 0.04,
        0,
        4
      );
      return {
        ...
        developedLike: Number((aggregate.perceptiveCount * 1.5 + aggregate.developedCount + aggregate.clearCount * 0.28).toFixed(2))
      };
    }
    ...
    function buildBoundaryReasons(aggregate, mark, inputState) {
      ...
      if (mark === 8) {
        reasons.push("The response sustains perceptive, grounded analysis with judicious evidence.");
      } else if (mark === 7) {
        reasons.push("The response is strong and interpretive, but not consistently judicious or sustained enough for 8.");
        if (aggregate.evidence.judiciousCount < 2) reasons.push("Evidence is selective rather than fully judicious.");
        if (aggregate.threadData.sustainedCount < 2) reasons.push("The conceptual thread is strong, but not sustained often enough for 8.");
      } else if (mark === 6) {
        reasons.push("The response is mainly clear and developed rather than consistently perceptive.");
        if (aggregate.threadData.sustainedCount < 1 || aggregate.coverage.interpretiveGroups.length < 2) reasons.push("There is not enough sustained conceptual interpretation for Level 4.");
      } else if (mark === 5) {
    ...
    function determineRuleMark(aggregate, inputState, dimensionScores) {
      ...
      const bandScores = {
        1: 0.5 + aggregate.referenceOnlyCount * 0.3 + aggregate.genericCount * 0.26 + aggregate.quoteDumpCount * 0.48 + aggregate.literalCount * 0.12,
        2: dimensionScores.explanationQuality * 0.9 + dimensionScores.evidenceQuality * 0.42 + dimensionScores.coverageQuality * 0.3 + aggregate.simpleCount * 0.18 + aggregate.literalCount * 0.18 - aggregate.unsupportedCount * 0.65 - aggregate.quoteDumpCount * 0.34,
        3: dimensionScores.explanationQuality * 1.2 + dimensionScores.evidenceQuality * 0.84 + dimensionScores.coverageQuality * 0.68 + dimensionScores.accuracy * 0.44 + dimensionScores.control * 0.34 + aggregate.clearCount * 0.38 + aggregate.developedCount * 0.24 + aggregate.simpleCount * 0.1 - aggregate.unsupportedCount * 0.75 - aggregate.quoteDumpCount * 0.28 - aggregate.featureSpottingCount * 0.18,
        4: dimensionScores.interpretationDepth * 1.48 + dimensionScores.explanationQuality * 0.78 + dimensionScores.evidenceQuality * 0.68 + dimensionScores.coverageQuality * 0.6 + dimensionScores.accuracy * 0.56 + dimensionScores.control * 0.24 + aggregate.perceptiveCount * 0.52 + aggregate.developedCount * 0.4 + aggregate.threadData.sustainedCount * 0.42 - aggregate.unsupportedCount * 1.0 - aggregate.seriousMethodMisuseCount * 0.36 - aggregate.featureSpottingCount * 0.34 - aggregate.literalCount * 0.12
      };

      const level4T = thresholds.level4 || {};
      if (aggregate.coverage.interpretiveGroups.length < (level4T.minInterpretiveCoverage || 2)) bandScores[4] -= 0.82;
      if (aggregate.threadData.sustainedCount < (level4T.minSustainedThreads || 1)) bandScores[4] -= 0.72;
      if (aggregate.evidence.usableCount < (level4T.minUsableEvidence || 2)) bandScores[4] -= 0.72;
      if (aggregate.evidence.selectiveCount < (level4T.minSelectiveEvidence || 1)) bandScores[4] -= 0.28;
      if (developedLike < (level4T.minDevelopedLike || 1.9)) bandScores[4] -= 1.05;
      if (weakRatio > (level4T.maxWeakRatio || 0.5)) bandScores[4] -= 0.72;
      if (aggregate.perceptiveCount === 0 && aggregate.threadData.sustainedCount === 0) bandScores[4] -= 0.52;

      const level3T = thresholds.level3 || {};
      if (aggregate.coverage.supportedGroups.length < (level3T.minSupportedCoverage || 1)) bandScores[3] -= 0.76;
      if (aggregate.evidence.attachedCount < (level3T.minAttachedEvidence || 1)) bandScores[3] -= 0.76;
      if (clearEquivalent < (level3T.minClearEquivalent || 1.55)) bandScores[3] -= 0.85;
      if (aggregate.clearCount + aggregate.developedCount + aggregate.perceptiveCount < 2) bandScores[3] -= 0.62;
      if (clearEquivalent < 1.35 && aggregate.evidence.usableCount < 2 && dimensionScores.weakRatio > 0.55) bandScores[3] -= 1.25;
      if (aggregate.referenceOnlyCount >= 2 && aggregate.genericCount >= 2) bandScores[3] -= 0.95;
      ...
      if (
        chosenBand === 4 &&
        (
          bandScores[4] - bandScores[3] < 0.42 ||
          (aggregate.perceptiveCount === 0 && aggregate.developedCount < 2) ||
          (aggregate.threadData.sustainedCount < 1 && aggregate.perceptiveCount === 0) ||
      ...
      if (
        chosenBand === 3 &&
        bandScores[3] - bandScores[2] < 0.28 &&
        clearEquivalent < 2.0 &&
      ...
      let rawMark = 0;
      if (chosenBand === 4) {
        rawMark = 6.85 + Math.min(1.05,
          aggregate.perceptiveCount * 0.2 +
          aggregate.developedCount * 0.1 +
          aggregate.evidence.judiciousCount * 0.22 +
          aggregate.threadData.sustainedCount * 0.18 +
          Math.max(0, dimensionScores.interpretationDepth - 3.05) * 0.16 +
          (aggregate.coverage.analyticalGroups.length >= 4 ? 0.08 : 0)
        );
      } else if (chosenBand === 3) {
        rawMark = 5.0 + Math.min(0.96,
          aggregate.developedCount * 0.22 +
          aggregate.perceptiveCount * 0.26 +
          aggregate.coverage.interpretiveGroups.length * 0.08 +
          aggregate.threadData.sustainedCount * 0.12 +
          aggregate.evidence.selectiveCount * 0.1 +
          Math.max(0, dimensionScores.explanationQuality - 2.6) * 0.12
        );
        ...
        rawMark = (aggregate.referenceOnlyCount + aggregate.genericCount + aggregate.quoteDumpCount + aggregate.literalCount) > 0 ? 1.68 : 1.0;
      }
      ...
      if (
        ...
        aggregate.threadData.sustainedCount >= ((floors.level4 || {}).minSustainedThreads || 999) &&
        aggregate.unsupportedCount === 0 &&
        rawMark < 6
      ) {
        rawMark = 6.0;
      ...
      if (
        ...
        (aggregate.evidence.judiciousCount === 0 || aggregate.threadData.sustainedCount < 1)
      ) {
        rawMark = 6.35;
        notes.push("The response is strong and interpretive, but it stays closer to developed Level 3 than secure Level 4.");
      }
      const secureEight = (
        (
        ...
        (
          ...
          aggregate.threadData.sustainedCount >= 2 &&
          (aggregate.evidence.selectiveCount >= 1 || aggregate.evidence.judiciousCount >= 1) &&
          weakRatio <= 0.26
        )
      );
      if (secureEight && rawMark < 7.6) {
        rawMark = 7.72;
      }
      if (
        rawMark > 7.35 &&
        ...
        rawMark = 7.35;
        notes.push("The response is very strong, but not fully sustained enough for 8.");
      }

      let rawPenalty = 0;
      if (aggregate.unsupportedCount > 0) rawPenalty += aggregate.unsupportedCount >= 2 ? 0.95 : 0.45;
      if (aggregate.contradictionCount > 0) rawPenalty += 0.35;
      if (aggregate.seriousMethodMisuseCount > 1) rawPenalty += 0.32;
      if (aggregate.hardUncertaintyCount > 1) rawPenalty += 0.25;
      if (aggregate.quoteDumpCount > 1 && aggregate.clearCount + aggregate.developedCount + aggregate.perceptiveCount === 0) rawPenalty += 0.4;
      ...
      if (rawMark > 4.35 && aggregate.featureSpottingCount > 0 && aggregate.evidence.usableCount === 0 && aggregate.perceptiveCount === 0 && aggregate.developedCount === 0) {
        rawMark = 4.35;
      }
      if (rawMark > 4.35 && aggregate.perceptiveCount === 0 && aggregate.developedCount === 0 && clearEquivalent < 1.4 && aggregate.evidence.usableCount <= 1) {
        rawMark = 4.35;
      }
      if (rawMark > 4.35 && aggregate.perceptiveCount === 0 && aggregate.developedCount === 0 && aggregate.clearCount <= 1 && aggregate.literalCount >= 1 && aggregate.featureSpottingCount >= 1 && aggregate.evidence.usableCount <= 1) {
        rawMark = 4.35;
      }
      if (rawMark > 3.35 && aggregate.featureSpottingCount >= 3 && aggregate.evidence.usableCount <= 1 && clearEquivalent < 2.0) {
        rawMark = 3.35;
      }
      if (rawMark < 2 && aggregate.unsupportedCount > 0 && aggregate.coverage.referenceGroups.length >= 1 && !aggregate.copyingStrong) {
        rawMark = 2.05;
      }
      if (rawMark < 3 && aggregate.featureSpottingCount > 0 && aggregate.coverage.referenceGroups.length >= 2 && !aggregate.copyingStrong) {
        rawMark = 3.05;
      }
      if (rawMark < 3 && aggregate.literalCount >= 1 && aggregate.coverage.referenceGroups.length >= 2 && !aggregate.copyingStrong) {
        rawMark = 3.02;
      }
    ...
    function determineMark(baseResult, helperPrediction) {
      ...
      if (helperPrediction && helperRules.enabled) {
        ...
        const canShiftUp = (
          diff > 0 &&
          helperPrediction.confidence >= (helperRules.upwardThreshold || 0.68) &&
          aggregate.unsupportedCount === 0 &&
        ...
        const canShiftDown = (
          diff < 0 &&
          helperPrediction.confidence >= (helperRules.downwardThreshold || 0.56)
        );

        if (canShiftUp) {
          const boost = Math.min(0.52, Math.max(0, helperPrediction.rawMark - ruleMarkData.mark));
          ...
          if (ruleMarkData.mark >= 8 && helperPrediction.suggestedMark <= 7 && helperPrediction.confidence >= 0.58 && aggregate.perceptiveCount < 2 && (aggregate.evidence.judiciousCount < 2 || aggregate.threadData.sustainedCount < 2)) {
          ...
          if (ruleMarkData.mark >= 7 && helperPrediction.suggestedMark <= 6 && helperPrediction.confidence >= 0.56 && ((aggregate.perceptiveCount === 0 && aggregate.developedCount <= 2 && aggregate.threadData.sustainedCount < 1) || aggregate.featureSpottingCount > 0)) {
          ...
          if (ruleMarkData.mark >= 6 && helperPrediction.suggestedMark <= 5 && helperPrediction.confidence >= 0.75 && aggregate.perceptiveCount === 0 && aggregate.threadData.sustainedCount === 0) {
          ...
          if (ruleMarkData.mark >= 4 && aggregate.unsupportedCount > 0) {
            finalRaw = Math.max(0, finalRaw - 0.25);
    ...
    function buildAggregate(units, copyMetrics) {
      ...
      aggregate.clearEquivalent = Number((aggregate.perceptiveCount * 1.62 + aggregate.developedCount * 1.28 + aggregate.clearCount + aggregate.simpleCount * 0.58 + aggregate.literalCount * 0.22).toFixed(2));
      aggregate.copyingStrong = (
        copyMetrics.wholeDice >= (rules.copyThresholds || {}).wholeDice ||
        copyMetrics.copiedUnitRatio >= 0.8 ||
        copyMetrics.longestRun >= (rules.copyThresholds || {}).longRun
        ...
        "<section class=\"grid-two\">" +
          ...
          "<div class=\"card\"><h3>Copy / extract checks</h3>" +
            "<dl class=\"kv\">" +
              ...
              "<div><dt>Library status</dt><dd>" + (result.libraries.compromise ? "Compromise on" : "Compromise off") + " · " + (result.libraries.winkNLP ? "winkNLP on" : "winkNLP off") + " · " + (result.libraries.Fuse ? "Fuse on" : "Fuse off") + "</dd></div>" +
            "</dl>" +
          "</div>" +
        "</section>" +
        "<section class=\"grid-two\">" +
    ...
    function bindUI(marker) {
      ...
      if (status) {
        ...
        status.textContent =
          ...
          " · " +
          (state.Fuse ? "Fuse loaded" : "Fuse unavailable") +
          " · " +
